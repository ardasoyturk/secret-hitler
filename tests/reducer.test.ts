...
describe("Game Reducer", () => {
  ...
  describe("Chaos (3 failed elections)", () => {
    test("3 failed elections trigger chaos — top policy enacted", () => {
      ...
      expect(state.phase === GamePhase.ChaosPolicy || state.phase === GamePhase.GameOver).toBe(
        true,
      );
  ...
  describe("Legislative Session", () => {
    ...
    test("chancellor enacts a policy", () => {
      ...
      expect(state.phase === GamePhase.PolicyEnacted || state.phase === GamePhase.GameOver).toBe(
        true,
      );
